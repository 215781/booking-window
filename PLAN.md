...
- [ ] **Summer data status check** — `_data/prices_clubmed_summer.csv` had **0 data rows** as of 2026-05-18 (header only). Combos crash bug fixed 2026-05-17 (commit 7fc1677); first real data run expected 2026-05-18 07:30 UTC. Monitor `_data/prices_clubmed_summer.csv` for first rows — resorts to expect: GREC, MMAC, DBAC, CARC, LAPC, LPAC, PALC, TURC, AGAC, KANC. Once data arrives, verify RESORT_DATA_SUMMER renders correctly in the Summer tab of `/clubmed`.
